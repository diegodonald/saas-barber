...
test.describe('🚀 Sistema SaaS Barber - Fluxo Completo', () => {
  ...
  test('Fluxo de Autenticação: Registro → Login → Logout', async ({ page }) => {
    ...
    await page.fill('[data-testid="name-input"]', testData.client.name);
    await page.fill('[data-testid="email-input"]', testData.client.email);
    await page.fill('[data-testid="password-input"]', testData.client.password);
    await page.fill('[data-testid="phone-input"]', testData.client.phone);
   ...
   */
  test('Gestão de Barbeiros: Cadastrar → Associar Serviços → Definir Horários', async ({ page }) => {
    ...
    await page.route('**/api/auth/**', async (route) => {
      ...
      if (url.includes('/login') && method === 'POST') {
        ...
        currentUser = {
          ...
          email: body.email,
          name: role === 'ADMIN' ? 'Admin Teste' : 
                role === 'BARBER' ? 'João Barbeiro' : 'Carlos Cliente',
          role: role,
  ...
  async function loginAs(page: Page, role: 'ADMIN' | 'BARBER' | 'CLIENT') {
    const credentials = {
      ADMIN: { email: 'admin@barbershop.com', password: 'senha123' },
      BARBER: { email: 'barbeiro@barbershop.com', password: 'senha123' },
      CLIENT: { email: 'cliente@email.com', password: 'senha123' }
    };
